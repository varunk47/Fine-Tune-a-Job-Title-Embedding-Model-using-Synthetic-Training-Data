...
if __name__ == "__main__":
    # Skip training and just copy the best checkpoint to streamlit location
    
    # Set paths
    build_project_path = r'C:\fine-tuning-build-project\Gemini2.5pro\fine-tuning-build-project'
    
    # Source checkpoint path (using checkpoint-232 which appears to be the best one)
    checkpoint_path = os.path.join(build_project_path, 'fine_tuning', 'data', 'trained_models', 'checkpoint-232')
    
    # Target path for streamlit app
    streamlit_model_path = os.path.join(build_project_path, 'streamlit_app', 'data', 'fine_tuned_model')
    ...
    # Copy the checkpoint to the streamlit app location
    print(f"Copying checkpoint from {checkpoint_path} to {streamlit_model_path}")
    
    # Option 1: If you want to copy the entire checkpoint directory
    if os.path.exists(streamlit_model_path):
        shutil.rmtree(streamlit_model_path)  # Remove existing directory if it exists
    shutil.copytree(checkpoint_path, streamlit_model_path)
    
    print(f"Checkpoint copied successfully to {streamlit_model_path}")
